...
class RulerView : View {
    ...
    companion object {
        ...
        /**
         * 刻度尺类型
         * SCALE_TEN 每20个刻度写一个刻度值
         *
         */
        const val SCALE_TWENTY = 0
    ...
    private val mMaxVelocity = ViewConfiguration.get(context).scaledMaximumFlingVelocity
    ...
    // 刻度尺画刻度值规则
    var mScaleLineTextType = SCALE_TWENTY
        set(value) {
            if (field != value) {
                field = value
                postInvalidate()
    ...
    // 当前值，默认 100
    var currentValue = if (mScaleType == TYPE_RULE) "100" else "10"
        set(value) {
            if (field != value) {
                field = value
                if (isRefresh) {
                    setValue(field.toFloat() - value.toFloat())
                    postInvalidate()
                } else {
                    isRefresh = true
                }
     ...
     * 中线画笔
     */
    var mMidLineColor = Color.parseColor("#333333")
        set(value) {
            if (field != value) {
                field = value
                mMidLinePaint.color = mMidLineColor
                postInvalidate()
            }
        }
     ...
     * 长线颜色宽度
     */
    var mLongLineWidth = dpToPx(2F).toInt()
        set(value) {
            if (field != value) {
                field = value
     ...
     * 短线颜色宽度
     */
    var mShortLineWidth = dpToPx(0.1F).toInt()
        set(value) {
            if (field != value) {
                field = value
     ...
     * 长线长
     */
    private var mLineHeight = dpToPx(24F).toInt()

    /**
     * 刻度线之间的最小间距
     */
    var mLineSpace = dpToPx(8F).toInt()
        set(value) {
            if (field != value) {
                field = value
                setSize()
    ...
    private val mMidLinePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style = Paint.Style.STROKE
    ...
    var mIndicatorSrc = R.mipmap.pointer
        set(value) {
            if (field != value) {
                field = value
                postInvalidate()
     ...
     * 刻度值文字画笔
     */
    private var mTextSize = dpToPx(14F)
    ...
    private var onValueChangedListener: ((value: String) -> Unit)? = null
    ...
    constructor(context: Context, attributeSet: AttributeSet?, defStyleAttr: Int) : super(
        ...
        mScaleType = typedArray.getInt(R.styleable.RulerView_msv_scale_type, mScaleType)
        mScaleLineTextType =
            typedArray.getInt(R.styleable.RulerView_msv_scale_line_able, mScaleLineTextType)
        mClickSelectAble =
            typedArray.getBoolean(R.styleable.RulerView_msv_click_select, mClickSelectAble)
        mIndicatorSrc =
            typedArray.getResourceId(R.styleable.RulerView_msv_indicator_src, mIndicatorSrc)
        mIndicatorBitmap = BitmapFactory.decodeResource(resources, mIndicatorSrc)
        val unitVal = typedArray.getFloat(R.styleable.RulerView_msv_unit, mUnitValue)
        ...
        mMin = typedArray.getInt(R.styleable.RulerView_msv_min, mMin)
        mMax = typedArray.getInt(R.styleable.RulerView_msv_max, mMax)
        val value = typedArray.getString(R.styleable.RulerView_msv_value) ?: ""
        // 重新计算默认值
        currentValue = if (mScaleType == TYPE_RULE) {
            try {
        ...
        val irregulars = typedArray.getString(R.styleable.RulerView_msv_irregulars)
        irregulars?.let {
        ...
        mSelectColor = typedArray.getColor(R.styleable.RulerView_msv_select_color, mSelectColor)
        mLongLineColor =
            typedArray.getColor(R.styleable.RulerView_msv_scale_long_line_color, mLongLineColor)
        mLongLineWidth = typedArray.getDimensionPixelSize(
            R.styleable.RulerView_msv_scale_long_line_width,
            mLongLineWidth
        )

        mMidLineColor =
            typedArray.getColor(R.styleable.RulerView_msv_scale_mid_line_color, mMidLineColor)

        mShortLineColor =
            typedArray.getColor(R.styleable.RulerView_msv_scale_short_line_color, mShortLineColor)
        mShortLineWidth = typedArray.getDimensionPixelSize(
            R.styleable.RulerView_msv_scale_short_line_width,
            mShortLineWidth
        )
        mLineHeight = typedArray.getDimensionPixelSize(
            R.styleable.RulerView_msv_scale_line_height,
            mLineHeight
        )
        mLineSpace =
            typedArray.getDimensionPixelSize(R.styleable.RulerView_msv_scale_line_space, mLineSpace)
        mTextColor = typedArray.getColor(R.styleable.RulerView_msv_scale_text_color, mTextColor)
        mTextSize = typedArray.getDimension(R.styleable.RulerView_msv_scale_text_size, mTextSize)
        mRecentlyNums =
            typedArray.getInt(R.styleable.RulerView_msv_min_scrollable_nums, mRecentlyNums)
        typedArray.recycle()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val mHeight =
            (paddingTop + paddingBottom + measureTextHeight() * 3 + mLineHeight / 2 * 3).toInt()
            ...
            for (index in 0..lineTotal) {
                ...
                if (isInVisibleArea(startX)) {
                    ...
                    if (mOnlyTenScaleLineAble) {
                        ...
                        canvas.drawLine(
                            startX.toFloat(),
                            paddingTop + mLineHeight / 2f,
                            startX.toFloat(),
                            paddingTop + mLineHeight / 2f * 3,
                            mLongLinePaint
                        )
                    } else {
                        when {
                            ...
                            index % 10 == 0 -> {
                                ...
                                canvas.drawLine(
                                    ...
                                    paddingTop + mLineHeight / 2f,
                                    startX.toFloat(),
                                    paddingTop + mLineHeight / 2f + mLineHeight / 4 * 3,
                                    mMidLinePaint
                                )
                            }
                            else -> {
                                ...
                                canvas.drawLine(
                                    ...
                                    paddingTop + mLineHeight / 2f,
                                    startX.toFloat(),
                                    paddingTop + mLineHeight.toFloat(),
                                    mShortLinePaint
                                )
                            }
                        }
    ...
    private fun drawScale(canvas: Canvas) {
        ...
        val baseline =
            paddingTop + mLineHeight / 2 * 3 + measureTextHeight() * 3 / 2 + (fontMetrics.descent - fontMetrics.ascent) / 2 - fontMetrics.descent
                    ...
                    for (index in 0..lineTotal) {
                        ...
                        if (isInVisibleArea(x)) {
                            if (mOnlyTenScaleLineAble) {
                                ...
                                if (index % 10 == 0) {
                                    mTextPaint.color =
                                        if (isSelectX(x)) mSelectColor else mTextColor
                                    canvas.drawText(
                                        calucateScaleValue(index),
                                        x.toFloat(),
                                        baseline,
                                        mTextPaint
                                    )
    ...
    private fun drawIndicator(canvas: Canvas) {
        //val scale = (mLineSpace * 2 - dpToPx(1F)) / mIndicatorBitmap.width
        val scale = 1.5F
    ...
    // 是否是fling的滑动的结束
    private var isFilingFinish = false
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                if (isClisckAble && mClickSelectAble && upTime <= 400) {
                    ...
                    mVelocityTracker?.computeCurrentVelocity(100, mMaxVelocity.toFloat())
                    //获取横向速率
                    ...
                    if (abs(velocityX) > mMinVelocity) {
                        ...
                        mScroller.fling(
                            ...
                            -velocityX,
                            0,
                            mLeftBorder - mShowWidth / 4 * 3,
                            mLeftBorder + mAllWidth - mShowWidth / 4,
                            0,
                            0
                        )
                        invalidate()
                        ...
                        when {
                            ...
                            mScroller.finalX > mLeftBorder + mAllWidth - mShowWidth / 2 -> {
                                doScroll(
                                    mLeftBorder + mAllWidth - mShowWidth / 2 - mScroller.finalX,
                                    500
                                )
                            }
                            else -> {
    ...
    private fun valueCallBack() {
        ...
        val value = when (mScaleType) {
            TYPE_RULE -> {
                val result = (((selectX - mMin) / mLineSpace).toFloat() * mUnitValue)
                when{
                    result <= mMin.toFloat() -> mMin.toFloat().toString()

                    result >= mMax.toFloat() -> mMax.toFloat().toString()
     ...
     * 设置当前值
     */
    private fun setValue(modifyValue: Float = 0F) {
        var dx = -1
        when (mScaleType) {
            TYPE_RULE -> {
                var value: Float
                ...
                //dx = (mLeftBorder + modifyValue / mUnitValue * mLineSpace).toInt() - mIndicatorX
                /*dx =
                    if (modifyValue == 0F) (mLeftBorder + (value - mMin) / mUnitValue * mLineSpace).toInt() - mIndicatorX
                    else (mLeftBorder + modifyValue / mUnitValue * mLineSpace).toInt() - mIndicatorX*/
            }
            TYPE_IRREGULAR -> {
                val index = if (mIrregularList.contains(currentValue)) {
                    mIrregularList.indexOf(currentValue)
                } else {
